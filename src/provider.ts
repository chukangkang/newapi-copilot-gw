...
export class HuggingFaceChatModelProvider implements LanguageModelChatProvider {
	...
	async provideLanguageModelChatResponse(
		...
		try {
			...
			logger.info("model.match", {
				modelId: model.id,
				parsedBaseId: parsedModelId.baseId,
				parsedConfigId: parsedModelId.configId,
				matchedModel: um?.id ?? "undefined",
				matchedApiMode: um?.apiMode ?? "undefined",
				totalUserModels: userModels.length,
				userModelIds: userModels.map((m) => m.id),
			});
...
interface OpenAIResponsesStatefulMarkerLocation { marker: string; index: number }
