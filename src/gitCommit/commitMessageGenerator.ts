	...
	if (selection.repo === null) {
		...
		for (const repo of reposWithChanges) {
			try {
				...
				console.error(`Failed to generate commit message for ${repo.rootUri.fsPath}`);
			}
		}
	} else {
...
async function promptRepoSelection(repos: GitRepository[]): Promise<RepoPickItem | undefined> {
...
async function performCommitMsgGeneration(secrets: vscode.SecretStorage, gitDiff: string, inputBox: vscode.SourceControlInputBox) {
